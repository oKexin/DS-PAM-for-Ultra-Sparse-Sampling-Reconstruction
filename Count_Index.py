from model import MRDN, RDN
from model_c import FD_UNet,SRResNet,ResUnet
import torch
import cv2
import numpy as np
from torchvision import transforms
import statistics
from utilis import PSNRCalculator
from torchmetrics.image import StructuralSimilarityIndexMeasure
import lpips
import warnings
warnings.filterwarnings("ignore", category=UserWarning)

def normalized(image):
    return (image - image.min()) / (image.max() - image.min())

def dowmsampling(image, ratio, flag = True):
    h, w = image.shape[:2]
    if flag:
        lr_patch = image[::ratio, ::ratio]
    else:
        lr_patch = np.zeros((h, w), dtype=image.dtype)
        lr_patch[::ratio, ::ratio] = image[::ratio, ::ratio]
    return lr_patch

def upsampling(image, ratio):
    h, w = image.shape[:2]
    sr_patch = cv2.resize(image, (w * ratio, h * ratio),
                          interpolation=cv2.INTER_CUBIC)
    return sr_patch

if __name__ == '__main__':
    scale_factor = 6
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model_index = 1 #1-proposed,2-woAR,3-woOR,4-FD_Unet,5-Res_Unet 6-SRGAN, 7-bicubic
    Dataset_Path = 'Result/test/data'
    Sample_Flag = True
    if model_index == 1:
        generator = MRDN(scale_factor=scale_factor).to(device)
        generator.eval()
        generator.load_state_dict(torch.load('Result/proposed_{}x.pth'.format(scale_factor), map_location=device))
        # generator.load_state_dict(torch.load('Result/woSE_{}x.pth'.format(scale_factor), map_location=device))
        # generator.load_state_dict(torch.load('Result/woDB_{}x.pth'.format(scale_factor), map_location=device))
    elif model_index == 2:
        generator = RDN(scale_factor=scale_factor).to(device)
        generator.eval()
        generator.load_state_dict(torch.load('Result/woAR_{}x.pth'.format(scale_factor), map_location=device))
    elif model_index == 3:
        generator = RDN(scale_factor=scale_factor).to(device)
        generator.eval()
        generator.load_state_dict(torch.load('Result/woOR_{}x.pth'.format(scale_factor), map_location=device))
    elif model_index == 4:
        generator = FD_UNet().to(device)
        generator.eval()
        generator.load_state_dict(torch.load('Result/FD_Unet_{}x.pth'.format(scale_factor), map_location=device))
        Sample_Flag = False
    elif model_index == 5:
        generator = ResUnet().to(device)
        generator.eval()
        generator.load_state_dict(torch.load('Result/Res_Unet_{}x.pth'.format(scale_factor), map_location=device))
        Sample_Flag = False
    elif model_index == 6:
        generator = SRResNet(upscale=scale_factor).to(device)
        generator.eval()
        generator.load_state_dict(torch.load('Result/SRGAN_{}x.pth'.format(scale_factor), map_location=device))
    transform = transforms.Compose([
        transforms.ToTensor(),
    ])
    total_ssim = []
    total_psnr = []
    total_lpips = []
    for num in range(1,7):
        or_path = 'testdata/{}_OR.png'.format(num)
        or_frame = cv2.imread(or_path, 0)
        if model_index == 1:
            ar_path = 'testdata/AR_Enh_{}x/{}_Enh.png'.format(scale_factor, num)
            # ar_path = 'testdata/AR_{}x/{}_Enh.png'.format(scale_factor, num) #w/o DB use
            ar_frame = cv2.imread(ar_path, 0)
            or_frame = normalized(or_frame).astype(np.float32)
            ar_frame = normalized(ar_frame).astype(np.float32)
            lor_frame = dowmsampling(or_frame, scale_factor, Sample_Flag)
            lor_frame = normalized(lor_frame).astype(np.float32)
            lor_frame = transform(lor_frame)
            ar_frame = transform(ar_frame)
            lor_frame = lor_frame.unsqueeze(0)
            ar_frame = ar_frame.unsqueeze(0)
            lor_frame = lor_frame.to(device)
            ar_frame = ar_frame.to(device)
            with torch.no_grad():
                hr_pred = generator(lor_frame, ar_frame)
        elif model_index == 2 or model_index == 4 or model_index == 5 or model_index == 6:
            or_frame = normalized(or_frame).astype(np.float32)
            lor_frame = dowmsampling(or_frame, scale_factor, Sample_Flag)
            lor_frame = normalized(lor_frame).astype(np.float32)
            lor_frame = transform(lor_frame)
            lor_frame = lor_frame.unsqueeze(0)
            lor_frame = lor_frame.to(device)
            with torch.no_grad():
                hr_pred = generator(lor_frame)
        elif model_index == 3:
            ar_path = 'testdata/AR_Enh_{}x/{}_Enh.png'.format(scale_factor,num)
            ar_frame = cv2.imread(ar_path, 0)
            ar_frame = normalized(ar_frame).astype(np.float32)
            ar_frame = transform(ar_frame)
            ar_frame = ar_frame.unsqueeze(0)
            ar_frame = ar_frame.to(device)
            with torch.no_grad():
                hr_pred = generator(ar_frame)
        else:
            down = dowmsampling(or_frame, scale_factor, Sample_Flag)
            hr_pred = upsampling(down, scale_factor)
            hr_pred = transform(hr_pred).unsqueeze(0).to(device)
        # Count Index
        hr_frame = normalized(or_frame).astype(np.float32)
        hr_frame = transform(hr_frame)
        result = hr_pred.squeeze().detach().cpu().numpy()
        result = normalized(result).astype(np.float32)
        index_result = transform(result).to(device)
        lpips_loss_fn = lpips.LPIPS(net='vgg',verbose=False).to(device)
        lpips_value = lpips_loss_fn(index_result.unsqueeze(0), hr_frame.unsqueeze(0).to(device)).item()
        psnr_count = PSNRCalculator().to(device)
        ssim_count = StructuralSimilarityIndexMeasure(data_range=1.0, reduction=None).to(device)
        psnr = psnr_count(index_result.unsqueeze(0), hr_frame.unsqueeze(0).to(device)).item()
        ssim = ssim_count(index_result.unsqueeze(0), hr_frame.unsqueeze(0).to(device)).item()
        print(f"{ssim:.4f}, {psnr:.2f}, {lpips_value:.4f}")
        total_ssim.append(ssim)
        total_psnr.append(psnr)
        total_lpips.append(lpips_value)
    maen_ssim = statistics.mean(total_ssim)
    maen_psnr = statistics.mean(total_psnr)
    maen_lpips = statistics.mean(total_lpips)
    var_ssim = statistics.pstdev(total_ssim)
    var_psnr = statistics.pstdev(total_psnr)
    var_lpips = statistics.pstdev(total_lpips)
    print(f"{maen_ssim:.4f}-{var_ssim:.4f}, {maen_psnr:.2f}-{var_psnr:.2f}, {maen_lpips:.4f}-{var_lpips:.4f}")


